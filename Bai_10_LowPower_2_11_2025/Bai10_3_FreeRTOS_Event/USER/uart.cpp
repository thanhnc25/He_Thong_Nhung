...
{
	if (u == USART1) return GPIOA; // PA9
	if (u == USART2) return GPIOA; // PA2
	if (u == USART3) return GPIOB; // PB10
	return GPIOA;
}
static uint16_t defaultTxPin(USART_TypeDef* u)
{
	if (u == USART1) return GPIO_Pin_9;  // USART1_TX
	if (u == USART2) return GPIO_Pin_2;  // USART2_TX
	if (u == USART3) return GPIO_Pin_10; // USART3_TX
	return GPIO_Pin_9;
}
static GPIO_TypeDef* defaultRxPort(USART_TypeDef* u)
{
	if (u == USART1) return GPIOA; // PA10
	if (u == USART2) return GPIOA; // PA3
	if (u == USART3) return GPIOB; // PB11
	return GPIOA;
}
static uint16_t defaultRxPin(USART_TypeDef* u)
{
	if (u == USART1) return GPIO_Pin_10; // USART1_RX
	if (u == USART2) return GPIO_Pin_3;  // USART2_RX
	if (u == USART3) return GPIO_Pin_11; // USART3_RX
	return GPIO_Pin_10;
}
...
bool Uart::Is_APB2() const { return usart_ == USART1; }

uint32_t Uart::Usart_Clock_APB() const
{
	if (usart_ == USART1) return RCC_APB2Periph_USART1;
	if (usart_ == USART2) return RCC_APB1Periph_USART2;
	if (usart_ == USART3) return RCC_APB1Periph_USART3;
	return 0;
}
